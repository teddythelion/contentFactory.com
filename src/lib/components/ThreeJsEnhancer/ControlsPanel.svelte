...
<script lang="ts">
	...
	$: controlGroups = [
		...
		},
		{
			...
			title: '🎨 Image Overlay',
			items: [
				{ label: 'Enable Logo', type: 'toggle' },
				{ label: 'Upload Logo', type: 'component', component: LogoUpload, componentProps: {} },
				...
				{ label: 'Auto Rotate Speed', type: 'range', min: 0.001, max: 0.1, step: 0.001 },
				{ label: '📍 Top Left', type: 'button', action: () => logoState.presets.topLeft() },
				{ label: '📍 Top Right', type: 'button', action: () => logoState.presets.topRight() },
				{ label: '📍 Bottom Left', type: 'button', action: () => logoState.presets.bottomLeft() },
				{ label: '📍 Bottom Right', type: 'button', action: () => logoState.presets.bottomRight() },
				{ label: '📍 Center', type: 'button', action: () => logoState.presets.center() },
	...
	function getControlValue(groupId: string, label: string): number | string | boolean {
		...
		if (groupId === 'logo') {
			switch (label) {
				case 'Enable Logo': return $logoState.enabled;
				...
				case 'Logo Animation': return $logoState.animationType;
	...
	function setControlValue(groupId: string, label: string, value: any) {
		if (groupId === 'logo') {
			...
			switch (label) {
				case 'Enable Logo': logoState.setEnabled(value); break;
				...
				case 'Logo Animation': logoState.setAnimation(value); break;
